...
ESC1=4  # right ESC connections with the GPIO pins; note its the BROADCOM number, not the GPIO pin number!
ESC2=17 # left
ESC3=27 # reverse
...
def control(): 
    ...
    time.sleep(1)
    speed_backward = 700    # change your speed if you want to.... it should be between 700 - 2000
    speed_right = 1000
    speed_left = 1000
    print ("Controls - a to decrease speed & d to increase speed OR q to decrease a lot of speed & e to increase a lot of speed")
    while True:
        pi.set_servo_pulsewidth(ESC1, speed_right)
        pi.set_servo_pulsewidth(ESC2, speed_left)
        pi.set_servo_pulsewidth(ESC3, speed_backward)
        inp = input()
        
        if inp == "r":
            speed_right += 100    # decrementing the speed a lot
            print ("speed = %d" % (speed_right,))
        elif inp == "e":
            speed_right -= 100    # decrementing the speed a lot
            print ("speed = %d" % (speed_right,))
        elif inp == "l":
            speed_left += 100     # incrementing the speed 
            print ("speed = %d" % (speed_left,))
        elif inp == "k":
            speed_left -= 100    # decrementing the speed a lot
            print ("speed = %d" % (speed_left,))
        elif inp == "b":
            speed_left = 1000# decrementing the speed
            speed_right = 1000
            speed_backward += 100
            print ("speed = %d" % (speed_right,))
        elif inp == "f":
            speed_left = 1200# decrementing the speed
            speed_right = 1200
            speed_backward = 700
            print ("speed = %d" % (speed_right,))
        elif inp == "R":
